...
#grid(
    ...
    [
        //左側
        == ▪︎ Contrastive Language-Image Pre-Training（CLIP）
...
#grid(
    ...
    [
        ...
        #v(0.0em) 
        == ▪︎ 提案手法における枝刈り
        - *Unified and Progressive Pruning（UPop）ベースのアプローチ*
        + マスクの勾配に基づいた枝刈り箇所の選択
        + 枝刈り率を段階的に増加・マスクの値を段階的に減少させることで性能維持を促進
        + 異なるモーダル及びモジュール（MLP，Attention）の枝刈り率を自動的に探索
        + 最終的にマスクの値を0に収束させることで枝刈りによるパラメータギャップを排斥
        #v(0.25em) 
        == ▪︎ 提案手法における知識蒸留
        - *同一モーダルと異なるモーダルの2つの観点からアプローチ*
...
- 定量的評価：各モーダルの特徴ベクトル（コサイン類似度・CKA類似度）・コサイン類似度行列（ピアソン相関係数）
    #figure(
    image("image/Quantitative_Evaluation.png",width: 80%)
    )],
    [
...
- データセット：COCO
#figure(
    image("image/ParamsFLOPs.png",width: 80%)
)
= 今後の展望
#sm[
